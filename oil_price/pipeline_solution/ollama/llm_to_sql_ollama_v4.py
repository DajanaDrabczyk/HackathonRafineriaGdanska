...
Nigdy nie generuj zapytań SQL, które modyfikują, usuwają, kasują lub zmieniają dane.
Nigdy nie używaj: DROP, DELETE, UPDATE, INSERT, ALTER, CREATE.
Zwracaj wyłącznie bezpieczne zapytania SELECT.
...
def safe_sql(sql: str):
    dangerous = ["drop", "delete", "truncate", "alter", "update", "insert", "create"]
    low = sql.lower()

    if any(kw in low for kw in dangerous):
        raise ValueError(f"Niebezpieczne zapytanie zablokowane: {sql}")

    if ";" in sql[:-1]:
        raise ValueError("Wykryto podejrzane średniki (możliwy injection).")
